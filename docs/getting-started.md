...
```bash
cd ~/Your-Research-Topic && /rk.init "your-research-topic"
```

Example:
```bash
cd ~/Your-Research-Topic && /rk.init "organizational AI transformation"
```

This creates a numbered project directory (e.g., `~/Your-Research-Directory-organizational-ai-transformation/`) with the complete structure:
...
```bash
/rk.init
cd ~/Your-Research-Topic && /rk.init "AI culture transformation"
```
...
1. **Initialize ResearchKit**: Run `/rk.init`
2. **Create your first project**: Run `cd ~/Your-Research-Topic && /rk.init "your-topic"`
